...
(defun memrise/parse-session-thing-texts (json pool)
  (let* ((column-id (memrise/session-pool-text-column pool))
         (column (assoc-default column-id json))
         (text (assoc-default 'val column))
         (text-options (append (assoc-default 'choices column) nil)))
    (add-to-list 'text-options 'text)))
